...
(function genCodes() {
  ...
  console.debug('validPoke', validPoke);
  const resCodes = [];
  App.game.discord.codes.forEach(e => e.claimed = false);
  const discordID = Math.floor((Math.random() * 65536) + 1);
  App.game.discord.ID(discordID);
  for (const codeString of validPoke) {
    const codeSeed = codeString.split('').reverse().map(l => l.charCodeAt(0)).reduce((s, b) => s * (b / 10), 1);
    console.debug('codeSeed', codeString, codeSeed);
  ...
  console.debug('resCodes', resCodes);
})();

(function () {
  function initAutoSafari() {
    let autoSafariState = false;
    let autoSafariPickItemsState = loadSetting('autoSafariPickItemsState', true);
    let autoSafariThrowBaitsState = loadSetting('autoSafariThrowBaitsState', false);
    let autoSafariSeekUncaught = loadSetting('autoSafariSeekUncaught', false);
    let autoSafariSeekContagious = loadSetting('autoSafariSeekContagious', false);
    let autoSafariFastAnimationsState = loadSetting('autoSafariFastAnimationsState', false);

    let scriptState = -1;
    let targetCoords = [];
    let cachedPath = [];
    let stopAfterGameOver = false;
    // To skip targets generated in buggy sections of the grid (theoretically can't happen anymore)
    let forceSkipItems = false;
    let forceSkipShinies = [];
    // To avoid constantly checking the spawn list. -1 for confirmed none, 1 for confirmed some
    let hasPrioritySpawns = 0;
    // Cached encounter tile(s) with highest priority spawn chance
    let cachedPriorityEnvironments = [SafariEnvironments.Grass];
    let mapHasTileType = {};
  ...
  function loadEpheniaScript(scriptName, initFunction, priorityFunction) {
    ...
    if (windowObject.epheniaScriptInitializers === undefined) {
      ...
      Preload.hideSplashScreen = function (...args) {
        let result = oldInit.apply(this, args);
